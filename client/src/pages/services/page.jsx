          ...
          <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-8">
            ...
            <div className="flex flex-wrap gap-4 justify-center md:justify-end">
              {skillTopics.map((topic, index) => (
                  <div 
                  key={index}
                  className="w-16 h-16 bg-[#1E1A3F] rounded-lg flex items-center justify-center p-3 hover:bg-[#2A2053] transition-all duration-300 ease-in-out transform hover:-translate-y-2"
                >
                  <img
                    src={topic.icon}
                    alt={`${topic.name} icon`}
